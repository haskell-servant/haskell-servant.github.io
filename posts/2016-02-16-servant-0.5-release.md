...
- We now use `ExceptT` (from `mtl`) rather than `EitherT` (from `either`).
...
and vanilla (xhr-based) codegen. There are [examples](https://github.com/haskell-servant/servant/tree/master/servant-js/examples)
of using these different systems in the repo.
